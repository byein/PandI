...
exports.in = function(request, response){ //alert가 안됨
        ...
        db.query(`SELECT * FROM member WHERE mId=?`, [post.userid], function(error, results){
                db.query(`SELECT * FROM admin WHERE aId=?`, [post.userid], function(error2, admin){
                        if (!results[0]){
                                if (!admin[0]){
...
exports.in_direct = function(request, response){ //alert가 안됨
        ...
        db.query(`SELECT * FROM member WHERE mId=?`, [userid], function(error, results){
                db.query(`SELECT * FROM admin WHERE aId=?`, [userid], function(error2, admin){
                        if (!results[0]){
                                if (!admin[0]){
                                        response.send('<script>alert("아이디 또는 비밀번호가 일치하지 않습니다."); window.location.href = `/login`;</script>');
                                } else {
                                        if(userpw != admin[0].aPwd){
                                                response.send('<script>alert("아이디 또는 비 밀번호가 일치하지 않습니다."); window.location.href = `/login`;</script>');
                                        } else {
                                        request.session.is_logined = true;
                                        request.session.name = admin[0].aId;
                                        request.session.pw = admin[0].aPwd;
