...
exports.create = function (request, response) {
    ...
    db.query(
        ...
        function (error, mem) {
            db.query(
                `SELECT aId FROM admin WHERE aId=?`,
                [post.userid],
                function (error2, admin) {
                    db.query(
                        `SELECT mEmail FROM member WHERE mEmail=?`,
                        [post.usermail],
                        function (error3, email) {
                            if (!mem[0] && !admin[0]) {
                                if (!email[0]) {
                                    db.query(
                                        `INSERT INTO member (mId, mPwd, mEmail, mDate, mName, mPost_code, mRoad_address, mJibun_address, mDetail_address, mExtra_address) VALUES (?, ?, ?, NOW(), ?, ?, ?, ?, ?, ?)`,
                                        [
                                            post.userid,
                                            post.userpw,
                                            post.usermail,
                                        ...
                                        function (error, result) {
                                            ...
                                            response.send(
                                                `<script>alert("회원가입이 완료되었습니다.");  window.location.href="/login_create/` +
                                                post.userid + `/` + post.userpw + `";</script>`
                                            );
                                        }
                                    );
                                } else {
                                    response.send(
                                        '<script>alert("이미 등록된 이메일입니다."); window.location.href = `/signUp`;</script>'
                                    );
                                }
                            } else {
                                response.send(
                                    '<script>alert("이미 등록된 아이디입니다."); window.location.href = `/signUp`;</script>'
                                );
                            }
                        }
                    );
